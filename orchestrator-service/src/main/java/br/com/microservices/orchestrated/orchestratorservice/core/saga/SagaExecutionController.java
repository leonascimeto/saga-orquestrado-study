...
import br.com.microservices.orchestrated.orchestratorservice.config.kafka.exception.ValidationException;
...
public class SagaExecutionController {
    ...
    public ETopics getNexTopic(Event event){
        ...
        var topic =  this.findTopicByStatusAndSource(event);
        logCurrentSaga(event, topic);
        return topic;
    }

    private ETopics findTopicByStatusAndSource(Event event){
    ...
    private void logCurrentSaga(Event event, ETopics topic){
        ...
        switch (event.getStatus()){
            case SUCCESS -> log.info("### CURRENT SAGA: {} | SUCCESS | NEXT_TOPIC {} | {}",
                    source, topic, sagaId);
            case ROLLBACK_PENDING -> log.info("### CURRENT SAGA: {} | SENDING TO ROLLBACK CURRENT SERVICE | NEXT_TOPIC {} | {}",
                    source, topic, sagaId);
            case FAIL -> log.info("### CURRENT SAGA: {} | SENDING TO ROLLBACK PREVIOUS SERVICE | NEXT_TOPIC {} | {}",
                    source, topic, sagaId);
        }
    }
